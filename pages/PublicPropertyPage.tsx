...
const PublicPropertyPage = () => {
            ...
            if (!error && propData) {
                ...
                if (propData.tenant_id) {
                    const { data: tenantData } = await supabase
                        .from('tenants')
                        .select('*') // Traemos todo para asegurar logo y contacto
                ...
                setPropiedad({
                    ...
                    // Aseguramos campos aunque vengan vacíos
                    recamaras: propData.features?.recamaras || 0,
                    banos_completos: propData.features?.banos_completos || 0,
                    terreno_m2: propData.features?.terreno_m2 || 0,
                    construccion_m2: propData.features?.construccion_m2 || 0,
                    descripcion_breve: propData.features?.descripcion_breve || '',
                    descripcion_general: propData.features?.descripcion_general || '',
                    tipo_inmueble: propData.tipo || 'Propiedad',
                    colonia: propData.features?.colonia || '',
                    municipio: propData.features?.municipio || '',
                    calle: propData.features?.calle || '',
                    numero_exterior: propData.features?.numero_exterior || '',
                    estado: propData.features?.estado || ''
                });
            }
            setLoading(false);
        };
    ...
    if (!propiedad) return <div className="min-h-screen flex items-center justify-center text-gray-500">Propiedad no encontrada.</div>;
            ...
            <header className="bg-white border-b sticky top-0 z-50 shadow-sm">
                <div className="max-w-6xl mx-auto px-4 py-3 flex justify-between items-center">
                    <div className="flex items-center gap-3">
                        {empresa?.logo_url ? (
                            <img src={empresa.logo_url} alt="Logo" className="h-10 object-contain" />
                        ) : (
                            <div className="text-xl font-bold text-gray-800">{empresa?.name || 'Inmobiliaria'}<span className="text-orange-500">.</span></div>
                        )}
                    </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
                    ...
                    <div className="space-y-4">
                        <div className="aspect-[4/3] w-full rounded-2xl overflow-hidden bg-gray-100 shadow-md relative group">
                            ...
                            {images.length > 1 && (
                                <>
                                    <button onClick={() => setActiveImage(prev => prev === 0 ? images.length -1 : prev - 1)} className="absolute left-3 top-1/2 -translate-y-1/2 bg-white/90 p-2 rounded-full text-gray-800 shadow-lg hover:scale-110 transition-all">❮</button>
                                    <button onClick={() => setActiveImage(prev => prev === images.length -1 ? 0 : prev + 1)} className="absolute right-3 top-1/2 -translate-y-1/2 bg-white/90 p-2 rounded-full text-gray-800 shadow-lg hover:scale-110 transition-all">❯</button>
                                </>
                            )}
                            <div className="absolute top-4 left-4 bg-orange-500 text-white px-3 py-1 rounded-md text-xs font-bold shadow-md uppercase tracking-wider">
                                {propiedad.tipo_inmueble} en Venta
                            </div>
                        </div>
                        {images.length > 1 && (
                    ...
                    <div>
                        ...
                        <div className="flex items-center gap-2 text-gray-500 mb-6 text-lg">
                            ...
                            <span>{propiedad.colonia}, {propiedad.municipio}</span>
                        </div>
                        ...
                        <div className="prose prose-orange text-gray-600 mb-8">
                            ...
                            <p className="leading-relaxed whitespace-pre-line">{propiedad.descripcion_breve || propiedad.descripcion_general || "Contáctanos para más información sobre esta propiedad."}</p>
                        </div>
                        ...
                        <div className="bg-gray-50 p-6 rounded-xl border border-gray-200 shadow-sm">
                            ...
                            <a 
                                href={`https://wa.me/${empresa?.telefono ? empresa.telefono.replace(/[^0-9]/g, '') : ''}?text=Hola, estoy viendo la propiedad ${propiedad.calle} en su sitio web y me interesa más información.`} 
                                target="_blank" 
