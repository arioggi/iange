...
const Input: React.FC<{ label: string; type?: string; required?: boolean; placeholder?: string; name: string; }> = ({ label, type = 'text', required, placeholder, name }) => (
    <div>
        ...
        <input type={type} name={name} id={name} placeholder={placeholder} required={required} className="w-full px-3 py-2 bg-gray-50 placeholder-gray-500 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-iange-orange focus:border-iange-orange sm:text-sm text-gray-900" />
    </div>
);
...
const Textarea: React.FC<{ label: string; required?: boolean; placeholder?: string; name: string; }> = ({ label, required, placeholder, name }) => (
    <div className="md:col-span-2">
        <label htmlFor={name} className="block text-sm font-medium text-gray-700 mb-1">{label}{required && <span className="text-red-500 ml-1">*</span>}</label>
        <textarea name={name} id={name} placeholder={placeholder} required={required} rows={3} className="w-full px-3 py-2 bg-gray-50 placeholder-gray-500 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-iange-orange focus:border-iange-orange sm:text-sm text-gray-900" />
    </div>
);
...
const Select: React.FC<{ label: string; required?: boolean; name: string; children: React.ReactNode; onChange?: (e: React.ChangeEvent<HTMLSelectElement>) => void }> = ({ label, required, name, children, onChange }) => (
    <div>
        <label htmlFor={name} className="block text-sm font-medium text-gray-700 mb-1">{label}{required && <span className="text-red-500 ml-1">*</span>}</label>
        <select name={name} id={name} required={required} onChange={onChange} className="w-full px-3 py-2 bg-gray-50 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-iange-orange focus:border-iange-orange sm:text-sm text-gray-900">
            {children}
        </select>
    </div>
);
...
const Facturacion: React.FC = () => {
    ...
    return (
        <div className="bg-white p-8 rounded-lg shadow-sm">
            <h2 className="text-2xl font-bold text-iange-dark mb-6">Facturación y Pagos</h2>
            <form onSubmit={handleSubmit}>
                ...
                <FormSection title="Opciones de pago">
                    ...
                    {metodoPago === 'Transferencia' && (
                        <>
                            <Input label="Nombre del titular" name="nombre_titular" placeholder="Asesores y Casas S.A. de C.V." />
                            <Input label="Cuenta bancaria (si aplica)" name="cuenta_bancaria" placeholder="BBVA 0123456789" />
                            <Input label="CLABE Interbancaria" name="clabe" placeholder="012345678901234567" />
                            <Input label="Banco" name="banco" placeholder="BBVA / Santander / Banorte" />
                        </>
                    )}
                </FormSection>
